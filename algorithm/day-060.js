...
function solution(number, limit, power) {
  let isPerfectSquare = Array(number).fill(false);
  const zeros = Array(number).fill(0);
  const numDivisors = zeros.map((numDiv, index) => {
    const number = index + 1;
    for (let divisor = 1; divisor ** 2 <= number; divisor++) {
      if (number % divisor === 0) numDiv++;
      if (divisor ** 2 === number) isPerfectSquare[index] = true;
    }
    return isPerfectSquare[index] ? numDiv * 2 - 1 : numDiv * 2;
  });
  const totalIronWeight = numDivisors.reduce((acc, curr) => {
    const ironWeight = curr > limit ? power : curr;

    return acc + ironWeight;
  });
