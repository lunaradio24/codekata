...
function solution(keymap, targets) {
  return targets.map((targetWord) => {
    // targetWord 치기 위한 최소 pressCount 초기화
    let pressCount = 0;
    for (let i = 0; i < targetWord.length; i++) {
      // targetWord를 이루는 알파벳 하나씩 numPresses 카운트 for all keys
      const numPresses = keymap.map((key) => {
        const index = key.indexOf(targetWord[i]);
        return index > -1 ? index + 1 : -1;
      });
      // numPresses 의 원소가 전부 -1이면 pressCount = -1 넣고 break;
      if (numPresses.every((num) => num === -1)) {
        pressCount = -1;
        break;
      }
      // 아니라면, numPresses 에서 -1을 제외하고 가장 작은 숫자를 pressCount에 추가
      else {
        const filteredNumPresses = numPresses.filter((num) => num > -1);
        pressCount += Math.min(...filteredNumPresses);
      }
    }
    // targetWord를 치기 위한 최소 pressCount를 반환
    return pressCount;
  });
}
